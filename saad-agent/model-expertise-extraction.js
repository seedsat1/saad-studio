...
export class ModelExpertiseExtractionService {
    static isExtractionRequest(prompt) {
    ...
    static resolveRequestedProvider(prompt) {
        const lower = String(prompt || "").toLowerCase();
        const normalized = this.normalizeArabic(prompt);
    ...
    static async extractFromGemini(params) {
        ...
        let responseText = "";
        try {
            responseText = EngineeringMemory.scrubSecrets(await ModelClient.chatCompletion(systemPrompt, userPrompt, runtime.model.modelName, {
    ...
    static async saveExpertiseCard(params) {
    ...
    static buildExpertisePrompts(topic, sourceModel) {
        const systemPrompt = [
    ...
    static async resolveRequestedProviderForRuntime(prompt) {
        const requested = this.resolveRequestedProvider(prompt);
        if (requested.id !== "gemini")
            return requested;
        try {
    ...
    static async getGeminiRuntimeForExtraction(signal) {
        const settings = await SettingsManager.getSettings();
        const provider = settings.providers.find((entry) => entry.id === "gemini" || entry.name.toLowerCase().includes("gemini"));
        if (!provider)
            throw new Error("Gemini provider is missing from Settings.");
        if (!provider.enabled)
            throw new Error("Gemini provider is disabled in Settings.");
        const apiKey = await SettingsManager.getProviderApiKey(provider);
        if (!apiKey)
            throw new Error("Gemini API key is missing. Add it in Settings > Providers > Gemini or set GEMINI_API_KEY.");
        const configuredModel = Object.values(settings.models).find((model) => model?.providerId === provider.id && model?.modelName);
        const discoveredModel = provider.discoveredModels?.find((model) => model?.id)?.id;
    ...
    static createUnavailableProviderResult(topic, provider) {
        return {
            saved: false,
            topic,
            provider,
...
//# sourceMappingURL=model-expertise-extraction.js.map