...
export class ResearchGatewayService {
    ...
    static async search(query) {
        ...
        for (const plannedQuery of plan.queries) {
            ...
            try {
                ...
                if (this.isConfigurationError(err))
                    throw err;
                failedQueries.push({
                    query: plannedQuery,
    ...
    static resolveProviderId(sources, usedAgentReach, usedDeepResearch) {
        const providers = new Set(sources.map((source) => source.provider));
        if (providers.size > 1)
            return "mixed";
        if (usedAgentReach && providers.has("agent-reach"))
            return "agent-reach";
        if (usedDeepResearch && providers.has("deep-research"))
    ...
    static formatConciseLinks(result) {
    ...
    static formatImageResults(result) {
    ...
    static isMediaSearchRequest(query) {
        return this.getMediaSearchKind(query) !== null;
    }
    static isImageSearchRequest(query) {
        return this.getMediaSearchKind(query) === "image";
    }
    static isSocialProfileSearchRequest(query) {
        if (this.hasLocalSearchScope(query))
            return false;
    ...
    static getMediaSearchKind(query) {
        if (this.hasLocalSearchScope(query))
            return null;
        const normalized = this.normalizeArabic(query);
        const lower = String(query || "").toLowerCase();
    ...
    static isGenericInternetFollowUp(query) {
        const normalized = this.normalizeArabic(query);
        const lower = String(query || "").toLowerCase();
    ...
    static hasSearchableTopic(query) {
    ...
    static formatSearchClarificationPrompt(query) {
    ...
    static buildResearchPlan(rawQuery) {
    ...
    static extractTargetSite(query) {
        const match = query.match(/https?:\/\/(?:www\.)?([^\/\s)>\]"]+)/i);
        return match?.[1]?.toLowerCase();
    }
    static extractCoreTerms(query, targetSite) {
    ...
    static isMeaningfulSearchTerm(term) {
    ...
    static inferExpansionTerms(originalQuery, coreTerms) {
    ...
    static hasLocalSearchScope(query) {
        const normalized = this.normalizeArabic(query);
        const lower = String(query || "").toLowerCase();
    ...
    static isGenericSearchTerm(term) {
        const normalized = this.normalizeArabic(term);
        return new Set([
    ...
    static extractSocialPlatform(query) {
        const normalized = this.normalizeArabic(query);
        const lower = String(query || "").toLowerCase();
        const haystack = `${normalized} ${lower}`;
        if (/(?:\u0627\u0646\u0633\u062a\u063a\u0631\u0627\u0645|\u0627\u0646\u0633\u062a\u06af\u0631\u0627\u0645|\u0627\u0646\u0633\u062a\u0643\u0631\u0627\u0645|\u0627\u0646\u0633\u062a\u0642\u0631\u0627\u0645|\u0627\u0646\u0633\u062a\u0627|instagram|insta)/i.test(haystack))
            return "instagram";
        if (/(?:facebook|\u0641\u064a\u0633\u0628\u0648\u0643|\u0641\u064a\u0633 \u0628\u0648\u0643)/i.test(haystack))
            return "facebook";
        if (/(?:tiktok|\u062a\u064a\u0643 \u062a\u0648\u0643|\u062a\u064a\u0643\u062a\u0648\u0643)/i.test(haystack))
            return "tiktok";
        if (/(?:twitter|x\.com|\u062a\u0648\u064a\u062a\u0631)/i.test(haystack))
            return "twitter";
        if (/(?:snapchat|\u0633\u0646\u0627\u0628)/i.test(haystack))
            return "snapchat";
        if (/(?:linkedin|\u0644\u064a\u0646\u0643\u062f\u0627\u0646|\u0644\u064a\u0646\u0643\u062f\u064a\u0646)/i.test(haystack))
            return "linkedin";
        return null;
    }
    static socialPlatformDomain(platform) {
        switch (platform) {
    ...
    static escapeMarkdownLabel(value) {
        return String(value || "")
            .replace(/[\[\]\n\r]/g, " ")
            .replace(/\s+/g, " ")
            .trim() || "Image";
    }
    static scoreSource(source, plan) {
        ...
        if (plan.targetSite && url.includes(plan.targetSite))
            score += 8;
        if (/\/(?:articles|models|tag|posts|images|collections|story|stories|forum|discussion|resources)\//i.test(source.url))
            score += 2;
        if (/support|login|privacy|terms|signin|signup|account|help/i.test(source.url))
            score -= 6;
        if (/^https?:\/\/(?:www\.)?[^/]+\/?$/i.test(source.url))
            score -= 2;
        return score;
    }
}
