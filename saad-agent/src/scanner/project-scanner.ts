...
export class ProjectScanner {
  ...
  private async scanFileHashes(): Promise<Record<string, string>> {
  ...
  async refresh(memoryStore: ProjectMemoryStore): Promise<boolean> {
  ...
  private removeFromArchitectureTree(root: ArchitectureNode, filePath: string): void {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (isLast) {
        current.children = current.children.filter((c: ArchitectureNode) => c.name !== part);
      } else {
        const next = current.children.find((c: ArchitectureNode) => c.name === part);
        if (!next) return;
        current = next;
      }
    }
  ...
  private pruneEmptyDirectories(node: ArchitectureNode): boolean {
    if (node.type !== "directory" || !node.children) {
      return false;
    }

    node.children = node.children.filter((child: ArchitectureNode) => {
  ...
  private updateArchitectureNode(root: ArchitectureNode, filePath: string): void {
    const parts = filePath.split("/").filter(Boolean);
    let current = root;
  ...
  private async updateDependenciesIncrementally(
  ...
  private async scanSummary(): Promise<ProjectSummary> {
    const packageJson = JSON.parse(await readFile("package.json"));
  ...
  private detectPackageManager(): string {
    try {
      if (existsSync(path.join(CONFIG.PROJECT_ROOT, "pnpm-lock.yaml"))) return "pnpm";
      if (existsSync(path.join(CONFIG.PROJECT_ROOT, "yarn.lock"))) return "yarn";
    } catch {}
    return "npm";
  }

  private detectBuildSystem(packageJson: any): string {
    if (packageJson.scripts?.build?.includes("next")) return "Next.js";
    if (packageJson.scripts?.build?.includes("vite")) return "Vite";
    if (packageJson.scripts?.build?.includes("tsc")) return "TypeScript";
    if (packageJson.scripts?.build?.includes("webpack")) return "Webpack";
    return "unknown";
  }

  private async scanArchitecture(): Promise<ArchitectureNode> {
  ...
  private addToArchitectureTree(root: ArchitectureNode, filePath: string): void {
    const parts = filePath.split("/").filter(Boolean);
    let current = root;
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      let child = current.children?.find((c: ArchitectureNode) => c.name === part);
      
      if (!child) {
        const newChild: ArchitectureNode = {
  ...
  private getFileType(filename: string): ArchitectureNode["type"] {
    if (filename.endsWith(".ts") || filename.endsWith(".tsx")) return "module";
    if (filename.endsWith(".js") || filename.endsWith(".jsx")) return "module";
  ...
  private async scanDependencies(): Promise<DependencyGraph> {
    const packageJson = JSON.parse(await readFile("package.json"));
