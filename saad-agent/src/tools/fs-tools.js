...
export async function listFiles(dirPath = ".") {
    ...
    for (const entry of entries) {
        if (entry.isDirectory()) {
            if (entry.name === "node_modules" || entry.name === ".git" || entry.name === ".next" || entry.name === ".saad-agent" || entry.name === "dist" || entry.name === "build")
                continue;
...
export async function searchText(pattern, dirPath = ".") {
