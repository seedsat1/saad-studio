...
function prettifyKnowledgeFileName(fileName) {
...
function deriveKnowledgeTitle(item) {
...
function inferTrainingCategoryForUrl(url, requestedCategory) {
...
function inferTrainingSubfolderForUrl(url, requestedCategory) {
    const value = `${url} ${requestedCategory || ""}`.toLowerCase();
    if (/(hotwife|cuckold|swinging|femdom|story|stories|lover|submission|relationship|psychology|intimacy|narrative)/i.test(value)) {
        return "stories";
    }
...
function safeUrlTrainingFileName(url) {
...
function describeTrainingFetchError(url, err) {
...
async function fetchTrainingUrlContent(url) {
...
function buildTrainingLinkMarkdown(url, category, tags) {
...
function buildCrawledTrainingMarkdown(url, category, tags, crawled, storyMode) {
...
function nextAvailableSync(basePath) {
...
ipcMain.handle("mcp-list-servers", async () => {
    try {
        ...
        const prompts = servers.flatMap((server) => server.prompts || []);
...
ipcMain.handle("knowledge:get-document", async (event, { id }) => {
    try {
        await KnowledgeManagerService.initialize();
        const document = readKnowledgeRegistry().find((item) => item.documentId === id || item.id === id);
...
ipcMain.handle("knowledge:get-term", async (event, { id, category }) => {
    try {
        ...
        for (const file of files) {
            ...
            const term = Array.isArray(terms) ? terms.find((item) => item.id === id || item.term === id) : null;
            if (term)
...
ipcMain.handle("knowledge:pack-export", async (event, { category }) => {
    try {
        await KnowledgeManagerService.initialize();
        const pack = KnowledgeManagerService.listPacks().find((item) => item.category === category || item.name === category);
...
async function setupApplicationMenu(win) {
