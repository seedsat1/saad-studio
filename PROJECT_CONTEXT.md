...
## Latest task: Enforce exclusive Admin-Only voice sample generation (2026-06-30)

- Status:
  Reconfigured `app/api/voice-sample/route.ts` to strictly disable any automatic on-the-fly paid generation of voice samples on GET request. If a subscriber requests a voice sample that has not been generated by the admin yet, the server redirects to a default voice sample (like "Sulafat") or returns a 404. Paid Gemini GenAI calls are now exclusively restricted to the Admin panel `/admin/voice-samples` via `POST /api/admin/voice-samples`, guaranteeing 0 dynamic billing cost for public previews.
- Affected files:
