...
## آخر مهمة: إبقاء Draw to Video وClipCraft Studio داخل مجموعة Video Studio فقط (2026-06-18)

- الحالة: رابطا `Draw to Video` و`ClipCraft Studio` موجودان داخل قائمة `VIDEO_FEATURES` (مجموعة Video Studio)، ومخفيان من `STUDIO_LINKS` (الروابط العلوية المباشرة). الصفحات والمسارات ما زالت متاحة.
- القرار: إبقاء الأداتين داخل المجموعة المنسدلة فقط لتقليل ازدحام الناف بار مع الحفاظ على إمكانية الوصول إليهما.
- التحقق: كل رابط موجود مرة واحدة فقط داخل `TopNavbar.tsx` في `VIDEO_FEATURES`، وفحص ESLint للملف نجح دون أخطاء.
- أخطاء مكتشفة: لا أخطاء جديدة؛ بقيت 3 تحذيرات سابقة من قاعدة `@next/next/no-img-element` في الملف نفسه.
- النشر: يُحفظ التصحيح في commit باسم `update` على فرع `main` ويُدفع إلى المستودع البعيد.
- الخطوة المتبقية: لا شيء. لم تتغير المعمارية أو آلية السلوك، لذلك لم يُحدّث مرجع Premiere/Reap.
