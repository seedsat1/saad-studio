...
## آخر مهمة: إخفاء Draw to Video وClipCraft Studio من الناف بار (2026-06-18)

- الحالة: أُزيل رابطا `Draw to Video` و`ClipCraft Studio` من قائمتي الفيديو والاستوديو في `components/TopNavbar.tsx` فقط؛ الصفحات والمسارات ما زالت متاحة.
- القرار: إخفاء عنصري التنقل بدل حذف الأدوات لأن الطلب يخص الناف بار فقط.
- التحقق: لا توجد مطابقة للاسمين أو المسارين داخل `TopNavbar.tsx`، وفحص ESLint للملف نجح دون أخطاء.
- أخطاء مكتشفة: لا أخطاء جديدة؛ بقيت 3 تحذيرات سابقة من قاعدة `@next/next/no-img-element` في الملف نفسه.
- النشر: تُحفظ تغييرات إخفاء الرابطين مع تحديث الذاكرة في commit باسم `update` على فرع `main` وتُدفع إلى المستودع البعيد.
