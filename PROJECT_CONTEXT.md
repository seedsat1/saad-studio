...
- **الإصلاح والتعديل**:
  ...
  2. تحديث `lib/providers/byteplus-video.ts` لربط المسار بمفتاح البيئة `BYTEPLUS_MODEL_MINI` مع تغيير قيم التراجع الافتراضية (Fallbacks) لتعتمد الهيكل المتوافق مع حساب العميل وهو `"dreamina-seedance-2-0-mini-260128"` (للـ Mini) و `"dreamina-seedance-2-0-lite-260128"` (للـ Fast) بدلاً من الأسماء القديمة غير المعرفة، لضمان العمل التلقائي.
