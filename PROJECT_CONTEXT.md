...
- Status:
  ...
  - Formatted omni_flash multimodal inputs in `lib/gemini-veo.ts` using source tags (`[# Sources <FIRST_FRAME>@Image1]`, `[# References <IMAGE_REF_0>@Image2]`) and explicit video configuration properties (`duration_seconds`, `resolution`, `aspect_ratio`, `task`) following Google's official REST schema.
  - Expanded `isMissingProviderTask` in `app/api/video/route.ts` to identify permanent API failures (400, 403, 401, unauthorized, forbidden, bad request) to immediately fail and refund stuck browser generations instead of looping indefinitely.
