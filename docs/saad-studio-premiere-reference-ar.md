...
- هذه التعريفات خاصة بعقل الـAgent والنصوص/المحادثة فقط، وليست ضمن موديلات الصور أو الفيديو أو الصوت أو 3D.
- الحالة التنفيذية بقيت `not_connected`: لا يوجد ربط إنتاجي مع `/api/director/v1`، ولا يوجد خصم كريديت أو استدعاء Gemini من هذا التغيير.
- الأسعار المخزنة هي أسعار مزود Google القياسية لكل مليون token نصي: Flash-Lite بسعر 0.10 دولار إدخال و0.40 دولار إخراج، وFlash بسعر 0.30 دولار إدخال و2.50 دولار إخراج، وPro بسعر 1.25 دولار إدخال و10 دولارات إخراج للطلبات حتى 200k tokens وبسعر 2.50 دولار إدخال و15 دولاراً إخراج لما فوق 200k tokens. الإخراج يشمل thinking tokens.
- ملاحظة تشغيلية: وثائق Google تشير إلى أن الوصول إلى موديلات Gemini 2.5 قد يكون محدوداً للمشاريع التي استخدمتها سابقاً، لذلك لا يعني التسجيل في الكتالوج أنها مفعّلة لحساب الإنتاج.

## إصلاح إضافة الكريديت اليدوية بعد انتهاء الصلاحية (2026-09-18)

- مسار الإدارة يسوّي الدورة السابقة للمستخدم المستهدف قبل الإضافة الموجبة، لمنع إحياء رصيد منتهٍ.
- الإضافة تستخدم `preserveExpiryOrFresh` الموجودة أصلًا: تحفظ نهاية الدورة السارية، أو تفتح نافذة 30 يومًا عند عدم وجود صلاحية سارية. الإضافة وسجلها المالي داخل المعاملة نفسها.
- إذا انتهى الاشتراك وأصبحت `monthlyCredits=0` مع نافذة رصيد جديدة سارية، لا يؤدي سجل الاشتراك القديم إلى تصفير هذه النافذة. تنتهي النافذة الجديدة في موعدها الطبيعي.
- لا تغيير في واجهات المستخدم أو مخطط قاعدة البيانات أو قواعد التجديد السنوي والسلف. لا إعادة منح تلقائية للأرصدة السابقة.
...
- **الهدف والمتطلبات**:
  - معالجة المشكلة المبلغ عنها من المستخدم: "لا يمكنني حفض الفديو في الهاتف" بعد الضغط على زر حفظ الفيديو في المعاينة على الهاتف.
- **التشخيص والتحليل الجذري**:
  1. على متصفح Safari (iOS) ومتصفحات الهواتف، تتطلب واجهة المشاركة النظامية `navigator.share` تفعيلاً متزامناً باللمس المباشر للمستخدم (User Activation).
     وكان الكود يقوم أولاً بتحميل ملف الفيديو بالكامل عبر `fetch` (الذي يستغرق ثوانٍ)، مما يؤدي لانتهاء صلاحية رمز اللمس الفوري وظهور خطأ `NotAllowedError`.
  2. عند حدوث الخطأ، كان الكود يتراجع إلى تنزيل الرابط عبر وسم `<a>` برابط `blob:`. ومن المعروف أن متصفح Safari على iOS يتجاهل خاصية التنزيل عبر `<a>` لملفات الفيديو تماماً ولا يقوم بتنزيلها.
  3. في راوت السيرفر المركزي للتنزيل `app/api/download/route.ts`، كان جلب الروابط النسبية `/api/media/...` يفشل برمز `ERR_INVALID_URL` لغياب النطاق `origin`.
- **الحلول المطبقة**:
  1. **الباك إند (`app/api/download/route.ts`)**:
     - دعم كامل للروابط النسبية بإضافة النطاق `req.nextUrl.origin` تلقائياً.
     - ضبط `Content-Type: video/mp4` مع `Content-Disposition: attachment` بصورة واضحة تجعل متصفح الجوال يُظهر نافذة التنزيل الرسمية للنظام فوراً.
  2. **مكتبة التنزيل على العميل (`lib/client-download.ts`)**:
     - إضافة بديل صريح للموبايل: في حال فشل أو تعذر المشاركة النظامية، يتم توجيه المتصفح فوراً لراوت المرفقات المباشر `/api/download` بدلاً من محاولة النقر الصامت غير الفعالة على `blob`.
  3. **نافذة معاينة وحفظ الفيديو (`app/(dash)/(routes)/m/gallery/page.tsx`)**:
     - إضافة حالة تحميل ومؤشر دوار أثناء تجهيز الفيديو لمنع النقر المتكرر.
     - توفير خيارين مستقلين وواضحين في النافذة:
       - الزر الأساسي: **"حفظ في ألبوم الصور (Photos / Share) 📲"** (يفتح قائمة مشاركة النظام لاختيار "Save Video / حفظ الفيديو" إلى تطبيق الصور مباشرة).
       - الزر الإضافي: **"تنزيل مباشر كملف MP4 📥"** (لتنزيل ملف الفيديو فوراً إلى مجلد التنزيلات بالجهاز).
     - إضافة إرشاد توضيحي للمستخدمين بالأسفل: "💡 على هواتف iPhone: اضغط 'حفظ في ألبوم الصور' ثم اختر 'Save Video' (حفظ الفيديو) من قائمة المشاركة لحفظه في تطبيق الصور فوراً."
- **التحقق الآلي**:
  - اجتياز جميع اختبارات `test/mobile-video-generation-contract.test.ts` (10/10 PASS).

## حل مشكلة عدم ظهور غلاف الفيديو (الثيمة) وإصلاح بطاقات المعرض للموبايل (2026-09-07)

- **الهدف والمتطلبات**:
  - معالجة المشكلة المبلغ عنها من المستخدم: "الفديوهات لمتظهر الثيمة" وظهور أيقونة الصورة المكسورة `[x]` والمربع الأسود مكان غلاف الفيديو في معرض الموبايل (`/m/gallery`).
- **التشخيص والتحليل الجذري**:
  1. في واجهة الموبايل `app/(dash)/(routes)/m/gallery/page.tsx`، كانت الدالة تعتمد بديلاً `thumbnailUrl: it.thumbnailUrl || it.posterUrl || mediaUrl`.
     وبالنسبة لملفات الفيديو، عند غياب `posterUrl`، كان المسار يمرر رابط الفيديو `.mp4` نفسه لخاصية `poster` في عنصر `<video>`.
     متصفحات الهواتف (مثل Safari و Chrome) تعامل خاصية `poster` كصورة نقطية حصرية، وعند استلام ملف من نوع `video/mp4` يفشل فاك الترميز في عرضه كصورة ويُظهر أيقونة الصورة التالفة (Broken image icon) مع خلفية سوداء بالكامل.
  2. في الباك إند `app/api/assets/route.ts`، كانت دالة `videoPosterUrl` تفحص فقط عمود `row.posterUrl`. وفي حال كان الرفع قيد التنفيذ أو لم تكتمل معالجة FFmpeg، لم تكن تفحص الصورة الأولية المرفوعة مع الطلب `startImageUrl` المحفوظة في `generationRequestSnapshot` (والتي تتوفر في كافة توليدات Image-to-Video).
- **الحلول المطبقة**:
  1. **الباك إند (`app/api/assets/route.ts`)**:
     - احتساب `effectiveVideoPoster`: إذا لم يتوفر `videoPoster` الجاهز في قاعدة البيانات، يتم اعتماد `startImageUrl` المحفوظة في الطلب فوراً كبوستر/غلاف عالي الدقة للفيديو.
     - إعادة `effectiveVideoPoster` في حقلي `posterUrl` و `thumbnailUrl` لأصول الفيديو، مع معالجة ذاتية (Self-healing) بتحديث السجل في قاعدة البيانات.
  2. **معرض الموبايل (`app/(dash)/(routes)/m/gallery/page.tsx`)**:
     - إضافة دالة حارسة تستبعد بصورة قاطعة ملفات الفيديو (`.mp4`, `.mov`, إلخ) من أن تُستخدم كبوستر.
     - في شبكة المعرض، إذا توفر البوستر كصورة، يتم عرضه عبر وسم `<img>` عالي السرعة والأداء وخفيف على معالج الهاتف مع lazy loading، مما يمنع ثقل تحميل وتشغيل الفيديوهات في الخلفية.
     - إذا لم يتوفر بوستر صورة، يتم الاعتماد على وسم الفيديو مع علامة `#t=0.001` المعتمدة لمتصفحات الجوال (WebKit/Safari) لالتقاط الفريم الأول تلقائياً وبدون أي خاصية `poster` خاطئة لمنع ظهور أيقونة الخطأ نهائياً.
     - تفعيل `playsInline` وخلفية أنيقة في نافذة المعاينة.
  3. **معرض سطح المكتب (`app/(dash)/(routes)/gallery/page.tsx`)**:
     - تفعيل استخدام `asset.thumbnailUrl` في البطاقات لتحسين سرعة التصفح وتوفير استهلاك الذاكرة.
- **التحقق الآلي**:
  - اجتياز جميع اختبارات `test/mobile-video-generation-contract.test.ts` (9/9 PASS).
  - اجتياز 32 اختباراً مستهدفاً لجميع الواجهات والموديلات (32/32 PASS).

## عزل تخطيط الموبايل عن شريط سطح المكتب ومزامنة مهام الفيديو قيد التوليد (2026-09-07)

- **الهدف والمتطلبات**:
  - معالجة التشوه البصري الناتج عن تداخل شريط سطح المكتب `TopNavbar` مع شريط الهاتف `MobileTopBar` داخل `/m/*`.
  - معالجة مشكلة عدم ظهور الفيديو المولد في المعرض في حال تنقل المستخدم من صفحة التوليد قبل اكتمال المعالجة.
- **التشخيص والتحليل الجذري**:
  1. **تشوه القائمة والشريط العلوي**:
     - شريط سطح المكتب `TopNavbar` المضمّن في `app/(dash)/layout.tsx` كان يظهر في صفحات `/m/*` مسبباً تراكب قائمتين معاً (القائمة الجانبية المنسدلة وشريط الهاتف `MobileTopBar`).
     - وجود مسافة `pt-16` داخل `<main>` كان يشوه أبعاد الواجهة المخصصة للموبايل.
  2. **عدم وصول الفيديو للمعرض**:
     - عند توليد الفيديو بنموذج Google Gemini/Veo أو WaveSpeed، يُنشأ الطلب بحالة `status: "processing"`.
     - كان مسار `/api/assets` يستبعد المهام غير المكتملة أو التي تحمل رابط `task:`، فكان المعرض يبدو فارغاً بالكامل ولا يُظهر للمستخدم أن الفيديو قيد التوليد.
     - في حال غادر المستخدم صفحة التوليد `/m/video`، كان الـ Polling يتوقف ولا تتم المزامنة من السيرفر إلا لموديل واحد (BytePlus).
- **الحلول المطبقة**:
  1. **عزل تخطيط الموبايل**:
     - تحويل `app/(dash)/layout.tsx` لفحص مسار الصفحة عبر `usePathname()`. إذا كان المسار يتبع `/m/*`، يُلغى ظهور `TopNavbar` و `PromotionRenderer` والبادئة `pt-16`.
     - إضافة حارس قاطع في `components/TopNavbar.tsx` لإرجاع `null` فوراً لأي مسار موبايل.
  2. **مزامنة ومعالجة المهام الجارية (In-Flight Reconciliation)**:
     - تصحيح دالة `decodeGeminiTask` في `lib/generation/task-reconciler.ts` لتدعم `base64url` و `base64` بدون أخطاء.
     - إضافة دالة `reconcileUserInFlightGenerations` واستدعاؤها تلقائياً داخل `app/api/assets/route.ts` عند فتح المعرض للتحقق من المهام الجارية للمستخدم واستكمالها وتخزينها على السيرفر فور انتهائها من المزود.
     - إرجاع المهام قيد المعالجة بعلامة `isProcessing: true` في `/api/assets`.
     - إظهار بطاقة تفاعلية حية ("جارٍ التوليد... ⏳") داخل `/m/gallery` مع مؤقت استطلاع دوري (كل 4 ثوانٍ) حتى يكتمل الفيديو ويتحول تلقائياً إلى بطاقة فيديو جاهزة للتشغيل والتنزيل.
- **التحقق الآلي**:
  - اجتياز اختبارات `test/mobile-video-generation-contract.test.ts` (8/8 PASS).
  - اجتياز كامل اختبارات الانحدار (35/35 PASS).
...
## مقاس النافبار العلوي للمشتركين (2026-09-23)

- ارتفاع `TopNavbar` المشترك هو `64px`، وهو مطابق لمساحة `pt-16` المحجوزة في تخطيطات الصفحات.
- عناصر التنقل الرئيسية تستخدم نصاً بحجم 14px وأيقونات بحجم 16px، مع شعار وزر قائمة موبايل بحجم 40px لتحسين القراءة والنقر.
- التعديل محصور في الصف العلوي؛ محتوى القوائم المنسدلة ومسارات الصفحات لا يتغيران.

## توجيه مراجع Seedance 2.5 (2026-09-23)

- الصور المضافة من لوحة **Add references** تبقى ضمن `reference_image_urls` ويستقبلها مسار Seedance 2.5 النصي/المرجعي، بحد أقصى 30 صورة وفق سجل النماذج في المشروع.
- لا تُحوّل الصور المرجعية العامة تلقائياً إلى صورتي بداية ونهاية.
- وضع صورة صراحةً في **Start Frame** يختار مسار `image-to-video` ويرسلها في `image`، ويمكن إرسال **End Frame** في `last_image`.
- مسار `image-to-video` لا يستقبل قائمة `reference_images`؛ الفصل السابق يمنع إرسال حمولة مختلطة يرفضها المزود.

## عقد قدرات موديلات Seedance الظاهرة (2026-09-23)

- الأسعار والكريديت لم تتغير.
- المراجع العامة لموديلات Seedance الستة تبقى على مسار `text-to-video`، بينما حقلا **Start Frame / End Frame** يوجهان الطلب إلى مسار `image-to-video` الموافق للموديل.
- الجودات:
  - Seedance 2.5 و2.0 Turbo و2.0 Mini Turbo: `720p`, `1080p`.
  - Seedance 2.0 و2.0 Mini و2.0 Fast: `480p`, `720p`, `1080p`, `4k`.
- المدد: Seedance 2.5 من 4 إلى 30 ثانية؛ موديلات 2.0 من 4 إلى 15 ثانية.
- النسب الرسمية الظاهرة: `16:9`, `9:16`, `1:1`, `4:3`, `3:4`, `21:9`. أزيل خيار `adaptive` لأنه غير موجود في عقود WaveSpeed التي تم التحقق منها.
- حدود المراجع: 2.5 حتى 30 صورة و10 فيديو و10 صوت؛ موديلات 2.0 حتى 9 صور و3 فيديو و3 صوت.
- في مسارات Image-to-Video الخاصة بـ2.0، عدم إرسال `aspect_ratio` يعني التكيف مع نسبة صورة البداية؛ لا تُرسل كلمة `adaptive` إلى المزود.
- في Seedance 2.5، عند وضع **Start Frame** تختفي أداة اختيار النسبة ولا يرسل العميل `aspect_ratio` لأن عقد I2V يتبع نسبة صورة البداية. عند استخدام Text/References تبقى النسب الست متاحة.
...
## سجل Agent / LLM المستقل (2026-09-24)

- `lib/agent-model-registry.ts` هو مصدر تعريف موديلات عقل الـAgent الثلاثة المعتمدة: `gemini-3.1-flash-lite` و`gemini-3.8-flash` و`gemini-3.1-pro-preview`.
- تعرض `/admin/models` تبويب Agent / LLM منفصلاً عبر `/api/admin/agent-models` المحمي بصلاحيات الأدمن. التعريفات محفوظة في الكود؛ لا تعتمد على إدراجها في قوائم Media أو على ترحيل قاعدة بيانات.
