...
2. **سلسلة التراجع (Fallback Chain)**:
   - **الخيار الأول (المفضل للملفات الجديدة)**: النطاق المباشر للـ Bucket في Backblaze B2 وهو `https://saadstudio-storage.s3.eu-central-003.backblazeb2.com` (أو الرابط العام الفائز من اختبارات الرفع).
   - **الخيار الثاني (الاحتياطي للملفات القديمة)**: النطاق الخام المباشر للـ R2 وهو `https://pub-3e0355a14eda4ec78c6e81b217a9a399.r2.dev` (مع استبعاد `media.saadstudio.app` حالياً بسبب مشاكل الـ DNS).
   - **الخيار الثالث (الطوارئ)**: البروكسي الداخلي الممر عبر Vercel وهو `/api/media` (يخدم Backblaze B2 افتراضياً، ويتراجع تلقائياً لجلب الملفات من R2 إذا لم تكن موجودة في B2). **ملاحظة استثنائية للفيديوهات**: معاينة وبث الفيديوهات (Preview/Stream) لا تستخدم هذا الخيار لمنع التحميل والـ buffering على Vercel، بينما يتاح البروكسي كـ fallback أخير فقط لسيناريوهات التنزيل (Download).
