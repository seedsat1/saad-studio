...
## Auto Captions للبودكاست ودعم العربية (2026-06-20)

- بطاقة `Auto Captions` داخل Podcast Automation ليست محركًا منفصلًا. هي مدخل إلى صفحة `Add Captions` الموجودة، حتى يبقى التوليد والتنزيل والاستيراد في مسار واحد ولا تتكرر حالات العمل.
- مدخل البودكاست يمرر `language=ar`. تقبل الصفحة لغة مطلوبة من الراوتر، وتثبتها إذا أعاد كتالوج اللغات دعمها؛ وعند تعذر الكتالوج يبقى `ar` موجودًا في القائمة الاحتياطية باسم `Arabic - العربية`.
- النص العربي يُطلب بالـNative script ولا يجوز قلب ترتيب الأحرف أو إجراء reshaping يدوي داخل CEP. ملف SRT يبقى UTF-8، ويُسلّم إلى Premiere كما أعاده مزود التفريغ.
- مسار Premiere الحالي: تنزيل `.srt` محليًا، ثم `placeCaptionFromSrt`، ثم `Sequence.createCaptionTrack` عند توفره. إذا تعذر إثبات التطابق مع الـSequence أو فشل إنشاء المسار، يُستورد SRT إلى Project Bin بدل وضعه عشوائيًا على Timeline.
- الأنماط البصرية المتحركة التي لا يمثلها SRT تستخدم ناتج الفيديو المصيّر؛ أما وضع `No style (SRT)` فهو المسار القابل للتحرير داخل Premiere.
- تعذر فتح صفحات Adobe الرسمية في جلسة 2026-06-20 بسبب HTTP 403/توقف تحميل المتصفح. لم تُستنتج API جديدة من هذا الفشل؛ تم تفعيل المسار الموجود الذي يضم host adapter وfallbacks وتشخيص الاستيراد بالفعل.
