...
## تصميم كارت البرومبت الموحد الشامل Composite Prompt Card (2026-07-25)

- تم إعادة بناء حقل مدخلات البرومبت بالكامل في استوديو الفيديو والصور (`/video`, `/image`) كـ Composite Prompt Card موحد ومنظم:
  1. **القسم العلوي داخل الكارت**: يعرض الشارات المرجعية وصور المعاينة (`@image1`, `@image2`, `@image3`) داخل الجزء العلوي للمربع مع خط فاصل أنيق.
  2. **المنطقة الوسطى المستقلة**: حقل كتابة النص `<textarea>` مخصص بالكامل في صف مستقل وواسع بكامل العرض (`min-h-[64px]` إلى `max-h-[220px]`) لمنع أي تداخل بين الكلمات المكتوبة وأزرار التحكم.
  3. **شريط الأدوات السفلي داخل الكارت**: شريط سفلي منظم يضم الأزرار التفاعلية (`Sparkles`, زر اختيار الشخصية `No character`) في اليسار، وزر المسح `X` وزر التوليد `Generate · 9 cr` في اليمين.
