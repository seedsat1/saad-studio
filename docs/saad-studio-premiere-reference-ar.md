...
- إذا عدّل الأدمن موديل `alibaba-wan-3.0-video` من `/admin/models`، فإن خيارات `aspect_ratios`, `durations`, `resolutions`, `max_reference_images`, `has_sound`, و`has_seed` المحفوظة في Admin Models تأخذ الأولوية وتظهر للمشترك في `/video`.
