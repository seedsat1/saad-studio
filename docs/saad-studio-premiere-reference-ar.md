...
1. **الهدف**: تجنب تمرير كافة أحمال الميديا (الصور، الفيديوهات الكبيرة، المقاطع الصوتية) عبر Vercel لتفادي حدود الحمولة وسرعة التنزيل والمهلات، والاعتماد على تسليم مباشر من R2.
2. **سلسلة التراجع (Fallback Chain)**:
   - **الخيار الأول (المفضل)**: النطاق المخصص المباشر للـ Bucket وهو `https://media.saadstudio.app` (بدون بروكسي أو تكلفة وسيطة).
   - **الخيار الثاني (الاحتياطي)**: النطاق الخام المباشر للـ R2 وهو `https://pub-3e0355a14eda4ec78c6e81b217a9a399.r2.dev`.
   - **الخيار الثالث (الطوارئ)**: البروكسي الداخلي الممر عبر Vercel وهو `/api/media` (يستخدم فقط كخط دفاع أخير إذا فشل الخيارين الأولين). **ملاحظة استثنائية للفيديوهات**: معاينة وبث الفيديوهات (Preview/Stream) لا تستخدم هذا الخيار لمنع التحميل والـ buffering على Vercel، بينما يتاح البروكسي كـ fallback أخير فقط لسيناريوهات التنزيل (Download).
3. **تطبيق الآلية في الواجهة الأمامية للموقع**:
   ...
   - يتوفر مراقب أخطاء عام (Global Capture Error Listener) في `app/layout.tsx` لاعتراض فشل تحميل أي عنصر وسائط وتبديله حياً لمنع تجميد واجهات المستخدم.
