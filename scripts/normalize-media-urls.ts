...
async function normalizeUserCharacterMediaUrls() {
  ...
  for (const char of chars) {
    ...
    if (Array.isArray(char.referenceUrls)) {
      newReferenceUrls = char.referenceUrls.map((url: unknown) => {
        if (typeof url === 'string') {
          const extracted = extractMediaPath(url);
          return extracted || url;
        }
        return url;
      }) as unknown as any; // Type assertion for Prisma JsonValue
    }
    ...
    if (needsUpdate) {
      await prisma.userCharacter.update({
        ...
        data: {
          ...
          referenceUrls: newReferenceUrls as any // Type assertion for Prisma
        }
      });
      updated++;
    }
