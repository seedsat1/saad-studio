    ...
    <div className="min-h-screen bg-[#060c18] text-white">
      {hero ? (
        <section
          ...
          style={{
            minHeight: 220,
            backgroundImage: heroMedia?.type === "image" && heroMedia.url ? `url(${heroMedia.url})` : undefined,
            backgroundSize: "cover",
            backgroundPosition: "center",
          }}
        >
          {heroMedia?.type === "video" && heroMedia.url ? (
            <video src={heroMedia.url} poster={heroMedia.poster} autoPlay muted loop playsInline preload="metadata" className="absolute inset-0 h-full w-full object-cover" />
          ) : null}
          <div className="absolute inset-0 bg-slate-950/70" />
          <div className="relative z-10 mx-auto max-w-6xl px-6 py-10">
            <span className="inline-flex rounded-full bg-violet-600 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-white">
              {hero.badge || "Character Library"}
            </span>
            <h1 className="mt-3 text-3xl font-bold">{hero.title || "Reusable AI Characters"}</h1>
      ...
      <main className="mx-auto grid max-w-7xl gap-6 px-4 py-6 lg:grid-cols-[390px_minmax(0,1fr)]">
        <section className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Upload clear views of the same person.</p>
            </div>
          </div>
        ...
        <section className="min-h-[560px] rounded-2xl border border-white/10 bg-white/[0.025] p-4">
