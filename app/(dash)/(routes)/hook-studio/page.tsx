...
export default function HookStudioPage() {
  ...
  const [effectActiveCategory, setEffectActiveCategory] = useState<"all" | "color" | "lighting" | "mood" | "action">("all");

  // Unified Reference Studio Modal State
  ...
  const [activeStudioTab, setActiveStudioTab] = useState<
    "history" | "uploads" | "stock" | "style" | "character" | "element" | "location" | "color" | "effects" | "camera" | "sketch"
  >("uploads");
  ...
  return (
    ...
    >
      ...
      {/* ───────────────────────────────────────────────────────────── */}
      {showReferenceStudioModal && (
        <div
          onClick={() => setShowReferenceStudioModal(false)}
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4 backdrop-blur-lg transition-all animate-in fade-in duration-200"
        >
          <div
            onClick={(e) => e.stopPropagation()}
            className="relative max-w-7xl w-full h-[90vh] bg-[#12151d] border border-slate-800/80 rounded-3xl overflow-hidden shadow-2xl flex flex-row transition-all animate-in zoom-in-95 duration-200 text-slate-100"
          >
            {/* ── 1. LEFT NAVIGATION RAIL (Sidebar Tabs) ── */}
            <div className="w-56 bg-[#161a24] border-r border-slate-800/70 p-4 flex flex-col justify-between flex-shrink-0 select-none">
              <div className="space-y-6">
                {/* Top Section: History & Uploads */}
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      <span className="flex-1 text-left">{isAr ? "صور الستوك" : "Stock"}</span>
                    </button>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span>{isAr ? "الستايل الفني" : "Style"}</span>
                      </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span>{isAr ? "الشخصيات والوجوه" : "Character"}</span>
                      </div>
                      <Pin className="w-3 h-3 text-slate-500 rotate-45" />
                    </button>
                    ...
                    {/* Element */}
                    <button
                      onClick={() => setActiveStudioTab("element")}
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-medium transition-colors ${
                        activeStudioTab === "element"
                          ? "bg-slate-800 text-white"
                          : "text-slate-400 hover:text-slate-200 hover:bg-[#1c2230]"
                      }`}
                    >
                      <Layers className="w-4 h-4 text-purple-400" />
                      <span className="flex-1 text-left">{isAr ? "العناصر والمنتجات" : "Element"}</span>
                    </button>
                    ...
                    >
                      ...
                      <span className="flex-1 text-left">{isAr ? "المواقع والخلفيات" : "Location"}</span>
                    </button>

                    {/* Color */}
                    <button
                      onClick={() => setActiveStudioTab("color")}
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-medium transition-colors ${
                    ...
                    >
                      ...
                      <span className="flex-1 text-left">{isAr ? "لوحة الألوان" : "Color"}</span>
                    </button>

                    {/* Effects */}
                    <button
                      onClick={() => setActiveStudioTab("effects")}
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-medium transition-colors ${
                    ...
                    >
                      ...
                      <span className="flex-1 text-left">{isAr ? "التأثيرات والإضاءة" : "Effects"}</span>
                    </button>
                    ...
                    >
                      ...
                      <span className="flex-1 text-left">{isAr ? "حركة وزوايا الكاميرا" : "Camera"}</span>
                    </button>

                    {/* Sketch */}
                    <button
                      onClick={() => setActiveStudioTab("sketch")}
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-medium transition-colors ${
                        activeStudioTab === "sketch"
                          ? "bg-slate-800 text-white"
                          : "text-slate-400 hover:text-slate-200 hover:bg-[#1c2230]"
                      }`}
                    >
                      <FileImage className="w-4 h-4 text-teal-400" />
                      <span className="flex-1 text-left">{isAr ? "الرسومات والسكيتشات" : "Sketch"}</span>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-800/60 text-[10px] text-slate-500 text-center">
                Saad Studio • Reference Engine
              </div>
            </div>

            {/* ── 2. CENTER MAIN CONTENT AREA ── */}
            <div className="flex-1 flex flex-col bg-[#0c0f16] overflow-hidden">
              {/* Header Bar */}
              <div className="p-5 border-b border-slate-800/60 bg-[#12151e]/90 flex items-center justify-between gap-4 flex-shrink-0">
                <div className="flex items-center gap-3">
                  <h2 className="text-base font-bold text-slate-100 capitalize">
                    {activeStudioTab === "uploads" && (isAr ? "الملفات المرفوعة (Uploads)" : "Uploads")}
                    {activeStudioTab === "history" && (isAr ? "السجل التاريخي (History)" : "History")}
                    {activeStudioTab === "stock" && (isAr ? "صور الستوك (Stock)" : "Stock Images")}
                    {activeStudioTab === "style" && (isAr ? "الستايل الفني (Style)" : "Art Style Presets")}
                    {activeStudioTab === "character" && (isAr ? "الشخصيات والوجوه (Character)" : "Character References")}
                    {activeStudioTab === "element" && (isAr ? "العناصر والمنتجات (Element)" : "Element References")}
                    {activeStudioTab === "location" && (isAr ? "المواقع والخلفيات (Location)" : "Location Environments")}
                    {activeStudioTab === "color" && (isAr ? "لوحة الألوان (Color)" : "Color Palettes")}
                    {activeStudioTab === "effects" && (isAr ? "التأثيرات والإضاءة (Effects)" : "Effects & Lighting")}
                    {activeStudioTab === "camera" && (isAr ? "حركة وزوايا الكاميرا (Camera)" : "Camera Motion")}
                    {activeStudioTab === "sketch" && (isAr ? "الرسومات والسكيتشات (Sketch)" : "Sketch & Line Art")}
                  </h2>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <div className="relative w-64">
                    <input
                      ...
                      className="w-full bg-[#161a25] border border-slate-800/80 rounded-xl pl-3 pr-8 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
                    />
              ...
              {/* Sub-Header Toolbar (Category Filters if Style or Effects active) */}
              {activeStudioTab === "style" && (
                <div className="px-6 py-3 bg-[#0d1018] border-b border-slate-800/50 flex items-center gap-2 flex-shrink-0">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all ${
                        styleActiveCategory === cat
                          ...
                          : "bg-[#161a25] text-slate-400 hover:text-slate-200"
                      }`}
                    >
                      {cat === "all" && (isAr ? "الكل" : "All")}
                      {cat === "illustration" && (isAr ? "رسومات Illustration" : "Illustration")}
                      {cat === "3d" && (isAr ? "ثلاثي الأبعاد 3D" : "3D")}
                      {cat === "design" && (isAr ? "تصميم وتصوير Design" : "Design")}
                    </button>
                  ))}
                </div>
              )}
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-[#080b11]">
                {/* ── TAB: UPLOADS & HISTORY ── */}
                {(activeStudioTab === "uploads" || activeStudioTab === "history") && (
                  <div className="space-y-8">
                    {/* July 2026 */}
                    <div>
                      <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">
                        {isAr ? "يوليو 2026 (July 2026)" : "July 2026"}
                      </h3>
                            ...
                            input.onchange = (e: any) => {
                              ...
                              if (f) {
                                ...
                                setAttachedFiles((prev) => [
                                  ...prev,
                                  { id: `up-${Date.now()}`, url, name: f.name, type: "image" },
                                ]);
                              }
                            };
                            input.click();
                          }}
                          className="aspect-square rounded-2xl border-2 border-dashed border-slate-800 hover:border-indigo-500 bg-[#0d1018] flex flex-col items-center justify-center text-slate-500 hover:text-indigo-400 transition-colors cursor-pointer"
                        >
                          <Plus className="w-6 h-6 mb-1" />
                          <span className="text-[10px] font-semibold">{isAr ? "رفع ملف" : "Upload"}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {activeStudioTab === "style" && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {HOOK_STYLES.filter((s) => {
                      ...
                      const search = studioSearchQuery.toLowerCase();
                      const matchesSearch = s.nameAr.toLowerCase().includes(search) || s.nameEn.toLowerCase().includes(search) || s.id.toLowerCase().includes(search);
                      ...
                      return (
                        <div
                          ...
                          className={`relative group rounded-2xl overflow-hidden border cursor-pointer transition-all duration-300 ${
                            isSelected ? "border-indigo-500 ring-2 ring-indigo-500/20 bg-indigo-500/5" : "border-slate-800 hover:border-slate-700 bg-[#0d1017] hover:bg-[#11151f]"
                          }`}
                        >
                          <div className="aspect-[4/3] w-full overflow-hidden relative bg-slate-900 border-b border-slate-800/60">
                            <img src={stylePreset.imageUrl} alt={stylePreset.nameAr} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            {isSelected && (
                          ...
                          <div className="p-3">
                            <div className="text-xs font-bold text-slate-200 truncate group-hover:text-indigo-400 transition-colors">
                              {isAr ? stylePreset.nameAr : stylePreset.nameEn}
                            </div>
                            <div className="text-[11px] font-semibold text-indigo-400 mt-0.5">{stylePreset.id}</div>
                          </div>
                        </div>
                      );
                    })}
                ...
                {activeStudioTab === "character" && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {HOOK_CHARACTERS.map((charPreset) => {
                      const isSelected = selectedCharacterId === charPreset.id;
                      return (
                        <div
                          key={charPreset.id}
                          onClick={() => {
                            if (isSelected) {
                              ...
                              setSelectedCharacterId(charPreset.id);
                              setAttachedFiles((prev) => [
                                ...prev,
                                { id: `char-${charPreset.id}-${Date.now()}`, url: charPreset.imageUrl, name: charPreset.nameAr, type: "image" },
                              ]);
                            }
                            setShowReferenceStudioModal(false);
                          }}
                          className={`relative group rounded-2xl overflow-hidden border cursor-pointer transition-all duration-300 ${
                            isSelected ? "border-amber-500 ring-2 ring-amber-500/20 bg-amber-500/5" : "border-slate-800 hover:border-slate-700 bg-[#0d1017] hover:bg-[#11151f]"
                          }`}
                        >
                          <div className="aspect-[4/3] w-full overflow-hidden relative bg-slate-900 border-b border-slate-800/60">
                            <img src={charPreset.imageUrl} alt={charPreset.nameAr} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            {isSelected && (
                              <div className="absolute top-3 right-3 bg-amber-500 text-white rounded-full p-1 shadow-md">
                                <Check className="w-3.5 h-3.5 stroke-[3]" />
                              </div>
                            )}
                          </div>
                          <div className="p-3">
                            <div className="text-xs font-bold text-slate-200 truncate group-hover:text-amber-400 transition-colors">
                              {isAr ? charPreset.nameAr : charPreset.nameEn}
                            </div>
                            <div className="text-[11px] font-semibold text-amber-400 mt-0.5">{charPreset.tag}</div>
                          </div>
                        </div>
                      );
                    })}
                        ...
                        <div
                          ...
                          onClick={() => {
                            if (isSelected) {
                              ...
                              setAttachedFiles((prev) => [
                                ...prev,
                                { id: `element-${elPreset.id}-${Date.now()}`, url: elPreset.imageUrl, name: elPreset.nameAr, type: "image" },
                              ]);
                            }
                            setShowReferenceStudioModal(false);
                          }}
                          className={`relative group rounded-2xl overflow-hidden border cursor-pointer transition-all duration-300 ${
                            isSelected ? "border-purple-500 ring-2 ring-purple-500/20 bg-purple-500/5" : "border-slate-800 hover:border-slate-700 bg-[#0d1017] hover:bg-[#11151f]"
                          }`}
                        >
                          <div className="aspect-[4/3] w-full overflow-hidden relative bg-slate-900 border-b border-slate-800/60">
                            <img src={elPreset.imageUrl} alt={elPreset.nameAr} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            {isSelected && (
                              <div className="absolute top-3 right-3 bg-purple-500 text-white rounded-full p-1 shadow-md">
                                <Check className="w-3.5 h-3.5 stroke-[3]" />
                              </div>
                            )}
                          </div>
                          <div className="p-3">
                            ...
                            <div className="text-[11px] font-semibold text-purple-400 mt-0.5">{elPreset.tag}</div>
                          </div>
                        </div>
                      );
                    })}
                        ...
                        <div
                          ...
                          onClick={() => {
                            if (isSelected) {
                              ...
                              setAttachedFiles((prev) => [
                                ...prev,
                                { id: `location-${locPreset.id}-${Date.now()}`, url: locPreset.imageUrl, name: locPreset.nameAr, type: "image" },
                              ]);
                            }
                            setShowReferenceStudioModal(false);
                          }}
                          className={`relative group rounded-2xl overflow-hidden border cursor-pointer transition-all duration-300 ${
                            isSelected ? "border-sky-500 ring-2 ring-sky-500/20 bg-sky-500/5" : "border-slate-800 hover:border-slate-700 bg-[#0d1017] hover:bg-[#11151f]"
                          }`}
                        >
                          <div className="aspect-[4/3] w-full overflow-hidden relative bg-slate-900 border-b border-slate-800/60">
                            <img src={locPreset.imageUrl} alt={locPreset.nameAr} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            {isSelected && (
                              <div className="absolute top-3 right-3 bg-sky-500 text-white rounded-full p-1 shadow-md">
                                <Check className="w-3.5 h-3.5 stroke-[3]" />
                              </div>
                            )}
                          </div>
                          <div className="p-3">
                            ...
                            <div className="text-[11px] font-semibold text-sky-400 mt-0.5">{locPreset.tag}</div>
                          </div>
                        </div>
                      );
                    })}
                ...
                {/* ── TAB: EFFECTS & COLOR ── */}
                {(activeStudioTab === "effects" || activeStudioTab === "color") && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {HOOK_EFFECTS.filter((eff) => {
                      const matchesCategory = activeStudioTab === "color" ? eff.category === "color" : (effectActiveCategory === "all" || eff.category === effectActiveCategory);
                      ...
                      return (
                        <div
                          ...
                          onClick={() => {
                            if (isSelected) {
                              ...
                              setAttachedFiles((prev) => [
                                ...prev,
                                { id: `effect-${effPreset.id}-${Date.now()}`, url: effPreset.imageUrl, name: effPreset.nameAr, type: "image" },
                              ]);
                            }
                            setShowReferenceStudioModal(false);
                          }}
                        ...
                        >
                          <div className="aspect-[4/3] w-full overflow-hidden relative bg-slate-900 border-b border-slate-800/60">
                            <img src={effPreset.imageUrl} alt={effPreset.nameAr} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            {isSelected && (
                              <div className="absolute top-3 right-3 bg-purple-500 text-white rounded-full p-1 shadow-md">
                                <Check className="w-3.5 h-3.5 stroke-[3]" />
                              </div>
                          ...
                          <div className="p-3">
                            ...
                            <div className="text-[11px] font-semibold text-purple-400 mt-0.5 flex items-center justify-between">
                              ...
                              <span className="text-[9px] uppercase tracking-wider text-slate-500 bg-slate-900 px-1.5 py-0.5 rounded">{effPreset.category}</span>
                            </div>
                          </div>
                        </div>
                      );
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {HOOK_CAMERAS.map((camPreset) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={() => {
                            if (isSelected) {
                              ...
                              setAttachedFiles((prev) => [
                                ...prev,
                                { id: `camera-${camPreset.id}-${Date.now()}`, url: camPreset.imageUrl, name: camPreset.nameAr, type: "image" },
                              ]);
                            }
                            setShowReferenceStudioModal(false);
                          }}
                        ...
                        >
                          <div className="aspect-[4/3] w-full overflow-hidden relative bg-slate-900 border-b border-slate-800/60">
                            <img src={camPreset.imageUrl} alt={camPreset.nameAr} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            {isSelected && (
                              <div className="absolute top-3 right-3 bg-amber-500 text-white rounded-full p-1 shadow-md">
                                <Check className="w-3.5 h-3.5 stroke-[3]" />
                              </div>
                          ...
                          <div className="p-3">
                            ...
                            <div className="text-[11px] font-semibold text-amber-400 mt-0.5">{camPreset.tag}</div>
                          </div>
                        </div>
                      );
                    })}
                ...
                {activeStudioTab === "sketch" && (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {HOOK_SKETCHES.map((skPreset) => {
                      const isSelected = selectedSketchId === skPreset.id;
                      return (
                        <div
                          key={skPreset.id}
                          onClick={() => {
                            if (isSelected) {
                              ...
                              setSelectedSketchId(skPreset.id);
                              setAttachedFiles((prev) => [
                                ...prev,
                                { id: `sketch-${skPreset.id}-${Date.now()}`, url: skPreset.imageUrl, name: skPreset.nameAr, type: "image" },
                              ]);
                            }
                            setShowReferenceStudioModal(false);
                          }}
                          className={`relative group rounded-2xl overflow-hidden border cursor-pointer transition-all duration-300 ${
                            isSelected ? "border-teal-500 ring-2 ring-teal-500/20 bg-teal-500/5" : "border-slate-800 hover:border-slate-700 bg-[#0d1017] hover:bg-[#11151f]"
                          }`}
                        >
                          <div className="aspect-[4/3] w-full overflow-hidden relative bg-slate-900 border-b border-slate-800/60">
                            <img src={skPreset.imageUrl} alt={skPreset.nameAr} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            {isSelected && (
                          ...
                          <div className="p-3">
                            <div className="text-xs font-bold text-slate-200 truncate group-hover:text-teal-400 transition-colors">
                              {isAr ? skPreset.nameAr : skPreset.nameEn}
                            </div>
                            <div className="text-[11px] font-semibold text-teal-400 mt-0.5">{skPreset.tag}</div>
                          </div>
                        </div>
                      );
                    })}
            ...
            {/* ── 3. RIGHT SIDEBAR PANEL (Custom Drag & Drop Uploader - Matches Screenshot) ── */}
            <div className="w-72 bg-[#12151e] border-l border-slate-800/70 p-6 flex flex-col items-center justify-center text-center flex-shrink-0 select-none">
              <div className="w-16 h-16 rounded-full bg-slate-800/60 flex items-center justify-center text-slate-400 mb-4 border border-slate-700/50 shadow-inner">
                <UploadCloud className="w-8 h-8 text-slate-300" />
              </div>

              <h4 className="text-xs font-medium text-slate-300 mb-6 leading-relaxed max-w-[180px]">
                {isAr ? "اسحب صورة أو ارفع ملف الوسائط الخاص بك" : "Drop an image or upload your own media"}
              </h4>
                  ...
                  onClick={() => {
                    ...
                    input.onchange = (e: any) => {
                      ...
                      if (f) {
                        ...
                        setAttachedFiles((prev) => [
                          ...prev,
                          { id: `custom-${Date.now()}`, url, name: f.name, type: "image" },
                        ]);
                      }
                    };
                    input.click();
                  }}
                  className="w-full bg-white hover:bg-slate-100 text-slate-900 font-bold text-xs py-2.5 px-4 rounded-xl shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
                  <UploadCloud className="w-4 h-4" />
                  <span>{isAr ? "رفع ملف ميديا" : "Upload media"}</span>
                </button>
                  ...
                  onClick={() => {
                    ...
                    input.onchange = (e: any) => {
                      ...
                      if (f) {
                        ...
                        setAttachedFiles((prev) => [
                          ...prev,
                          { id: `photo-${Date.now()}`, url, name: f.name, type: "image" },
                        ]);
                      }
                    };
                    input.click();
                  }}
                  className="w-full bg-[#1c2230] hover:bg-[#242b3d] text-slate-200 font-semibold text-xs py-2.5 px-4 rounded-xl border border-slate-700/60 transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
