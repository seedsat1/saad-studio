    ...
    if (routedModel.startsWith("google/lyria")) {
      ...
      try {
        ...
        if (!audioBase64) {
          ...
          const status = await failIdempotency({
            ...
            key: idempotencyKey,
            generationId,
            errorMessage: "No audio generated by Lyria model",
            providerDispatched,
          });
          if (status === "review_required") {
            return NextResponse.json({ generationId, status, error: "No audio generated by Lyria model" }, { status: 409 });
          }
        ...
        const status = await failIdempotency({
          ...
          key: idempotencyKey,
          generationId,
          errorMessage: `Lyria music generation failed: ${errMsg}`,
          providerDispatched,
        });
        if (status === "review_required") {
          return NextResponse.json({ generationId, status, error: `Lyria music generation failed: ${errMsg}` }, { status: 409 });
        }
      ...
      if (!externalRes.ok) {
        ...
        const status = await failIdempotency({
          ...
          key: idempotencyKey,
          generationId,
          errorMessage: `Music generation failed: ${detail}`,
          providerDispatched,
        });
        if (status === "review_required") {
          return NextResponse.json({ generationId, status, error: `Music generation failed: ${detail}` }, { status: 409 });
        }
      ...
      if (!waveUrl) {
        ...
        const status = await failIdempotency({
          ...
          key: idempotencyKey,
          generationId,
          errorMessage: "No audio URL in provider response",
          providerDispatched,
        });
        if (status === "review_required") {
          return NextResponse.json({ generationId, status, error: "No audio URL in provider response" }, { status: 409 });
        }
