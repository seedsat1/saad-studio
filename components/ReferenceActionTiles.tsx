...
import { Sparkles, User, Plus, X, Image as ImageIcon, Film, Volume2, Palette } from "lucide-react";
import {
  HOOK_STYLES,
  HOOK_ELEMENTS,
  HOOK_LOCATIONS,
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between gap-1.5 rounded-xl bg-black/20 p-1.5 border border-white/[0.03]">
          <div className="flex items-center gap-1">
            ...
            >
              <Film className={`w-4 h-4 transition-all ${hasVideos ? "text-purple-400" : "text-slate-500 group-hover:text-slate-400"}`} />
              <div
                className={`absolute bottom-0 left-1 right-1 h-[2px] rounded-full transition-all ${
                  hasVideos
            ...
            >
              <Volume2 className={`w-4 h-4 transition-all ${hasAudio ? "text-teal-400" : "text-slate-500 group-hover:text-slate-400"}`} />
              <div
                className={`absolute bottom-0 left-1 right-1 h-[2px] rounded-full transition-all ${
                  hasAudio
          ...
          <button
            type="button"
            onClick={onAddMedia}
            className="flex-1 h-[30px] rounded-lg border border-dashed border-slate-700 hover:border-cyan-500/80 bg-cyan-500/5 hover:bg-cyan-500/10 flex items-center justify-center gap-1 transition-all duration-200 cursor-pointer text-cyan-400 hover:text-white"
          >
            <Plus className="w-3.5 h-3.5" />
            <span className="text-[10px] font-bold tracking-tight">{isAr ? "إضافة وسائط" : "Add media"}</span>
          </button>
        </div>
