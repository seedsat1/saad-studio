...
export function InfluencerAssistantSidebar({ isOpen, onClose, onExecuteCommand }: InfluencerAssistantSidebarProps) {
  ...
  const assistantCopy = useMemo(() => {
    if (lang === "en") {
      return {
        hello:
          "Welcome. Describe the asset you want and I can route you to the right studio or canvas workflow.",
        received: (text: string) => `Received: "${text}". I am preparing the matching workflow step.`,
        title: "AI Assistant",
        question: "How can I help?",
        description:
          "Ask for images, video, motion control, face swap, VIP/NSFW variants, upscale, or library actions.",
        placeholder: "Describe what you want...",
      };
    }
    return {
      hello:
        "أهلا بك. صف الأصل الذي تريد إنشاءه وسأوجهك إلى الاستوديو أو سير العمل المناسب داخل الكانفاس.",
      received: (text: string) => `تم استلام طلبك: "${text}". جار تحضير خطوة العمل المناسبة.`,
      title: "مساعد الذكاء الاصطناعي",
      question: "كيف يمكنني المساعدة؟",
      description:
        "اطلب صورا أو فيديو أو نسخ حركة أو تبديل وجه أو نسخ VIP/NSFW أو رفع دقة أو إدارة المكتبة.",
      placeholder: "صف ما تريد إنشاءه...",
    };
  }, [lang]);
  const [prompt, setPrompt] = useState("");
  const [messages, setMessages] = useState<Array<{ role: "user" | "assistant"; text: string }>>([
    { role: "assistant", text: assistantCopy.hello },
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      setMessages((prev) => [...prev, { role: "assistant", text: assistantCopy.received(text) }]);
    }, 300);
  };

  return (
    <div className="fixed inset-y-0 right-0 z-40 w-80 sm:w-96 bg-[#0c0d16]/95 border-l border-pink-500/30 shadow-2xl backdrop-blur-2xl flex flex-col justify-between animate-in slide-in-from-right duration-300">
      <div className="h-16 px-5 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
        ...
        <div className="flex items-center gap-2 text-pink-400 font-bold text-sm">
          <span>{copy.assistant === "Assistant" ? assistantCopy.title : assistantCopy.title}</span>
          <Sparkles size={16} className="text-pink-400 animate-pulse" />
        </div>
      </div>

      <div className={`flex-1 overflow-y-auto p-5 space-y-4 scrollbar-none ${isArabic ? "text-right" : "text-left"}`}>
        <div className="flex flex-col items-center justify-center pt-6 pb-2 space-y-3 text-center">
          ...
          <h4 className="text-white font-bold text-base">{assistantCopy.question}</h4>
          <p className="text-xs text-zinc-400 leading-relaxed px-2">{assistantCopy.description}</p>
        </div>
          ...
          <div
            ...
            className={cn(
              ...
              msg.role === "user"
                ...
                : "ml-auto bg-white/5 text-zinc-200 border border-white/10"
            )}
          >
            {msg.text}
          </div>
      ...
      <div className="p-4 border-t border-white/10 bg-white/[0.01]">
        <div className="relative flex items-center bg-[#07080f] border border-pink-500/40 rounded-xl p-1 shadow-inner focus-within:border-pink-500">
          <input
            ...
            className={`w-full bg-transparent px-3 py-2 text-xs text-white placeholder-zinc-500 outline-none ${isArabic ? "text-right dir-rtl" : "text-left dir-ltr"}`}
          />
          <button
            onClick={handleSend}
