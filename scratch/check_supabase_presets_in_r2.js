...
async function main() {
  ...
  console.log("Checking if original Supabase videos exist in Cloudflare R2...");
  
  for (const [presetId, key] of Object.entries(originalPresets)) {
    try {
      await r2Client.send(new HeadObjectCommand({ Bucket: env.R2_BUCKET_NAME, Key: key }));
      console.log(`✅ ${presetId}: EXISTS in R2! ("${key}")`);
    } catch (e) {
      console.log(`❌ ${presetId}: NOT in R2! ("${key}")`);
    }
  }
}
