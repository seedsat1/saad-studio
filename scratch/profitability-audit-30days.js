...
// Inline pricing data from lib/pricing.ts
...
function getRealisticDefaultDuration(modelRef) {
  ...
  if (m.includes("reap") || m.includes("clipcraft")) {
    if (m.includes("transcription") || m.includes("captions")) return 180; // 3 minutes
    if (m.includes("edit")) return 300; // 5 minutes
...
function estimateProviderCostSync(modelRef, durationSec = 5, quality) {
  ...
  if (modelLower.includes("image") || modelLower.includes("banana") || modelLower.includes("rmbg") || modelLower.includes("upscale") || modelLower.includes("face-swap")) {
    return { usd: 0.01, source: "estimated" }; // flat $0.01 for images usually
  }
...
async function main() {
  ...
  console.log("Analyzing records since:", thirtyDaysAgo.toISOString());

  // Fetch transactions to compute credit pricing per user (for actual revenue recognition)
  ...
  allUsers.forEach((user) => {
    ...
    userCreditValues[user.id] = creditsGranted > 0 ? (totalPayments / creditsGranted) : 0.05; // default to list price
  });
  ...
  console.log(`Found ${generations.length} generations in the last 30 days.`);
  ...
  console.log("\n=== PROVIDER STATS (WITH REALISTIC ESTIMATES) ===");
  ...
  console.log("\n=== MODEL CATEGORY STATS (WITH REALISTIC ESTIMATES) ===");
