        ...
        for (var p = 0; p < props.length; p++) {
            ...
            if (isLinear) {
                for (var k = 0; k < keyTimes.length; k++) {
                    try {
                        var writeResult = entry.property.setInterpolationTypeAtKey(keyTimes[k], 0, true);
                        successfulKeys++;
                    } catch (e) {
                ...
                for (var k = 0; k < keyTimes.length - 1; k++) {
                    ...
                    try {
                        var val0 = entry.property.getValueAtKey(k1Time);
                        var val1 = entry.property.getValueAtKey(k2Time);
                        ...
                        for (var f = 1; f < totalFrames; f++) {
                            ...
                            entry.property.addKey(timeSec);
                            entry.property.setValueAtKey(timeSec, value, f === totalFrames - 1);
                        }

                        entry.property.setInterpolationTypeAtKey(k1Time, 1, true);
                        entry.property.setInterpolationTypeAtKey(k2Time, 1, true);
                        successfulKeys += 2;
                    } catch (e) {
    ...
    function saadKeysPproShift(deltaFrames, selectedIds) {
        ...
        for (var i = 0; i < props.length; i++) {
            ...
            for (var r = keyTimes.length - 1; r >= 0; r--) {
                try { entry.property.removeKey(keyTimes[r]); } catch (_) {}
            }
            for (var j = 0; j < keyTimes.length; j++) {
                var newTime = Number(keyTimes[j]) + deltaTicks;
                if (newTime < 0) newTime = 0;
                try {
                    entry.property.addKey(newTime);
                    if (vals[j] !== null) entry.property.setValueAtKey(newTime, vals[j], true);
                    moved++;
