...
export function DrawToVideoPage(): HTMLElement {
  ...
  function showStart() {
    body.replaceChildren(
      el("div.state-card", null,
        ...
        el("div.state-card__title", null, "Sketch a clip"),
        el("div.state-card__subtitle", null,
          "Grab the current frame from your timeline, draw what you want to change, and generate a clip."),
        el("div.state-card__actions", null,
          el("button.btn-primary", { onClick: captureFrame },
            icon("video", 14), "Capture current frame"),
        ),
      ),
    );
  }
  ...
  async function captureFrame() {
    ...
    try {
      ...
      const snap = await evalES<FrameSnapshot>("getActiveTimelineFrameSnapshot");
      if (!snap?.imagePath) throw new Error("No frame returned from host");
      showCanvas(snap);
    } catch (err) {
      body.replaceChildren(errorCard((err as Error).message));
    }
  }

  function showCanvas(snap: FrameSnapshot) {
    ...
    body.replaceChildren(
      el("div.col.gap-3", { style: { padding: "0 16px" } },
        el("div", { style: { borderRadius: "12px", overflow: "hidden" } }, canvas),
        el("div.row.gap-2", null,
          el("button.btn-secondary", {
            onClick: () => { ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.drawImage(bg, 0, 0); },
          }, "Clear"),
          el("button.btn-secondary", { onClick: showStart }, "Recapture"),
          el("button.btn-primary", { onClick: () => submit(canvas) },
            icon("send", 14), "Generate"),
        ),
      ),
    );
  }

  async function submit(canvas: HTMLCanvasElement) {
    try {
...
function busyCard(text: string): HTMLElement {
  return el("div.state-card", null,
    ...
    el("div.state-card__subtitle", null, "Hang tight."));
}

function errorCard(text: string): HTMLElement {
  return el("div.state-card", null,
    el("div.state-card__title", null, "Something went wrong"),
    el("div.state-card__subtitle", null, text));
}
