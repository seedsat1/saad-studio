...
export function ConnectPage(): HTMLElement {
  ...
  const step = (n: number, label: string, child?: HTMLElement) =>
    el("div.row.gap-3", { style: { alignItems: "flex-start", padding: "10px 0" } },
      el("div", {
        style: {
      ...
      el("div.col.gap-2.grow", null,
        el("div", { style: { fontSize: "13px", color: "var(--text-primary)" } }, label),
  ...
  return el("div.col",
    ...
    el("div", {
      ...
      el("div.row.gap-3", { style: { marginBottom: "16px" } },
      ...
      step(1, "Open your account page on saadstudio.app.",
        el("button.btn-secondary",
          { onClick: () => openExternal(`${API_BASE}/panel`) },
          icon("arrow-up-right", 12), "Open saadstudio.app",
        ),
      ),
      step(2, "Generate a panel token and copy it."),
      step(3, "Paste the token below and tap Connect.", input),
      el("div", { style: { marginTop: "16px" } }, connectBtn),
    ),
  );
}
