...
* **Comparative Agent References**: Claude Code and similar coding-agent systems may be used as comparative architecture references for exposed patterns such as terminal operation, codebase reading, file editing, shell execution, permission modes, MCP/tools, skills, memory, subagents, worktrees, reviews, and verification loops.
* **Leak Safety Boundary**: Repositories or archives described as leaked, proprietary, or unofficial must never be copied from, vendored, reverse-engineered into the product, or treated as licensed implementation material. They may only inform high-level architectural observation when the same pattern is also supported by official documentation or independent public research.
* **Commercial Release Cleanliness Gate**: If Saad Agent or any derivative product is prepared for sale, subscription, customer deployment, public distribution, or third-party delivery, all leaked/proprietary/unofficial reference archives and copied source snapshots must be excluded or deleted from release materials, training bundles, packaged artifacts, repositories, and build inputs before packaging. Commercial builds may contain only original Saad Agent code, user-owned code, or material with clear compatible licensing.
* **Claude-First Original Implementation Rule**: Saad Agent may prioritize Claude Code-like product behavior for design work, large-project maintenance, tool orchestration, command workflows, memory, permissions, sub-agents, plugins, skills, and verification loops. The implementation must remain original Saad Agent code and must be derived from local product requirements, official documentation, public research, and observed high-level patterns only.
