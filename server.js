...
app.get('/api/site-config.css', (req, res) => {
  try {
    const theme = fs.existsSync(THEME_FILE) ? JSON.parse(fs.readFileSync(THEME_FILE,'utf8')) : {};
    const fonts = fs.existsSync(FONTS_FILE) ? JSON.parse(fs.readFileSync(FONTS_FILE,'utf8')) : {};
    const vars  = { ...theme };
...
app.get('/api/admin/theme', requireAdmin, (req, res) => {
  try {
    const theme = fs.existsSync(THEME_FILE) ? JSON.parse(fs.readFileSync(THEME_FILE,'utf8')) : DEFAULT_THEME;
...
app.get('/api/admin/fonts', requireAdmin, (req, res) => {
  try {
    const fonts = fs.existsSync(FONTS_FILE) ? JSON.parse(fs.readFileSync(FONTS_FILE,'utf8')) : DEFAULT_FONTS;
...
// CMS_SYSTEM_START
const CMS_FILE = path.join(VAULT_DIR, 'cms.json');
...
function loadCMS(){try{if(!fs.existsSync(CMS_FILE))return getDefaultCMS();return JSON.parse(fs.readFileSync(CMS_FILE,'utf8'));}catch{return getDefaultCMS();}}
function saveCMS(d){fs.writeFileSync(CMS_FILE,JSON.stringify(d,null,2));}
...
app.get('/api/user/billing', requireSignedIn, (req, res) => {
  try {
    const u = req.currentUser;
    const db = loadOrders();
...
app.get('/api/user/orders', requireSignedIn, (req, res) => {
  try {
    const db = loadOrders();
    const myOrders = db.orders
